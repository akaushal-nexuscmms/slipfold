...
with sync_playwright() as p:
    ...
    # the return
    page.get_by_role("button", name=re.compile(r"7 · Your return")).click(); page.wait_for_timeout(400)
    body = page.inner_text("body")
    ...
    page.get_by_role("button", name=re.compile(r"8 · Next year")).click(); page.wait_for_timeout(300)
    body = page.inner_text("body")
    ...
    page.get_by_role("button", name=re.compile(r"7 · Your return")).click(); page.wait_for_timeout(500)
    body = page.inner_text("body")
    ...
    # ---------- every field on the forms ----------
    page.get_by_role("button", name=re.compile(r"5 · Every field")).click(); page.wait_for_timeout(300)
    body = page.inner_text("body")
    check("fields: page explains what the app fills and what is yours", "are yours to type" in body and re.search(r"\d+ of \d+ typed", body, re.I) is not None)
    check("fields: Elections Canada and foreign property questions offered", "Canadian citizenship" in body and "26600" in body)
    page.get_by_label("Search fields").fill("11300"); page.wait_for_timeout(200)
    body = page.inner_text("body")
    check("fields: search narrows to the OAS line", "Old age security" in body and "Canadian citizenship" not in body)
    page.locator("input[type=number]").first.fill("6001"); page.wait_for_timeout(400)
    page.get_by_label("Search fields").fill("citizenship"); page.wait_for_timeout(200)
    page.locator("select").last.select_option("yes"); page.wait_for_timeout(300)
    page.get_by_label("Search fields").fill(""); page.wait_for_timeout(200)
    check("fields: counter shows two typed", "2 of 137 typed" in page.inner_text("body").lower(), page.inner_text("body").lower()[:0])
    page.get_by_role("button", name=re.compile(r"7 · Your return")).click(); page.wait_for_timeout(400)
    body = page.inner_text("body")
    check("fields: typed OAS shows on the return as line 11300 with the unverified note", "11300" in body and "6,001.00" in body)
    page.get_by_role("button", name=re.compile(r"11300")).click(); page.wait_for_timeout(200)
    check("fields: expanded line says it was typed", "Every field page" in page.inner_text("body"))
    page.get_by_role("button", name=re.compile(r"5 · Every field")).click(); page.wait_for_timeout(300)
    page.get_by_label("Search fields").fill("11300"); page.wait_for_timeout(200)
    page.locator("input[type=number]").first.fill(""); page.wait_for_timeout(300)
    page.get_by_label("Search fields").fill(""); page.wait_for_timeout(200)
    page.get_by_role("button", name=re.compile(r"4 · Other amounts")).click(); page.wait_for_timeout(200)
    check("fields: no digital-news input on a 2025 return (line 31350 ended with 2024)", "31350" not in page.inner_text("body"))
    # ---------- tax year 2024 ----------
    page.get_by_role("button", name=re.compile(r"1 · You")).click(); page.wait_for_timeout(200)
    ...
    page.get_by_role("button", name=re.compile(r"4 · Other amounts")).click(); page.wait_for_timeout(200)
    check("years: digital-news input offered on the 2024 return", "31350" in page.inner_text("body"))
    opts = [o.inner_text() for o in page.locator("select").first.locator("option").all()]
    ...
    mpage.get_by_role("button", name=re.compile(r"7 · Your return")).click(); mpage.wait_for_timeout(400)
